...
int main(int argc, char *const *argv)
{
...
#if (HAVE_PCRE)
    ngx_regex_init();/*正则表达式初始化*/
#endif
    
    /*ngx_pid 是一个全局变量
      全局变量在ngx里有很多，都是以ngx_开头，
      可见：ngx_max_sockets = -1;也是全局变量
    */
    ngx_pid = ngx_getpid();/*逗逼设计，getpid()==ngx_getpid()*/

    if (!(log = ngx_log_init_stderr())) {
        /*ngx错误初始化
          ngx_log_t是一种数据结构，我在这里不详细说明，后面会继续讲解
        */
        return 1;
    }

#if (NGX_OPENSSL)
    ngx_ssl_init(log);/*初始化ngx_ssl_init*/
#endif
    ...
    /*
     *init_cycle ,ngx_cycle是局部的变量设计
       ngx_cycle_t是核心数据结构，后面会详细剖析
     */
    init_cycle.log = log;
    ...
    */
    ...
    /*
    ngx_create_pool,分配内存池，从这里开始，我开始分析内存池
    */
    if (!(init_cycle.pool = ngx_create_pool(1024, log))) {
        return 1;
    }
    /*
    将ctx里面的 argc,argv添加到核心数据init_cycle里
     */
   ...
   */
    ...
    */
    if (!ngx_inherited && ccf->daemon) {
        /*
          ngx_daemon函数的设计可以看出ngx核心团队人员有多么的保守，
          就是一个最标准的daemon进程
          （1）fork()一个子进程kill 死父进程
          （2）设置文件掩码,umask(0)可以随意更改文件属性，拥有文件的所有权
          （3）chdir，切换目录地址
           (4)开始会话，setsid（）
          （5）切断设备（0,1,2）
        */ 
        ...
        /*
           后面我会修改0.1的内核，将log_daemon进程设计为
           两次fork()模式，防止精灵进程有设备关联
        */
...
#endif
   /*
    开始核心的业务处理部分，如果是多进程 ngx_master_process_cycle(cycle, &ctx);
    如果是单进程 ngx_single_process_cycle(cycle, &ctx);
   */
